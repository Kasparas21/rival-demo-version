-- Run this entire script in Supabase SQL Editor (select all → Run).
-- Safe to re-run: drops and recreates the view.
...
select
  ...
  case
    when bs.status = 'trialing'
      and (
        ...
        or coalesce(bs.raw_payload->>'dev_plan_override', '') = 'pro'
      )
      and not (
        nullif(trim(coalesce(bs.raw_payload->>'tester_invite', '')), '') is not null
        or coalesce(bs.raw_payload->>'tester_claim_source', '') = 'complimentary'
    ...
    when coalesce(bs.raw_payload->>'admin_unlimited', 'false') = 'true'
      or coalesce(bs.raw_payload->>'dev_plan_override', '') = 'admin'
      then 'admin'
  ...
  (
    b.ads_profile_setup is not null
    and b.ads_profile_setup::text not in ('null', '{}')
  ) as has_ads_profile_setup,
