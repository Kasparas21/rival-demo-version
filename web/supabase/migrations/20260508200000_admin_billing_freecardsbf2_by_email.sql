-- Complimentary admin: resolves user from auth.users by email (UUID in older migrations may not match).
...
from auth.users u
where lower(trim(u.email)) = lower(trim('freecardsbf2@gmail.com'))
