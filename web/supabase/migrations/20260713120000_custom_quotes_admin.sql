-- Custom per-user pricing quotes, admin access, and pre-aggregated admin snapshots.
...
comment on table public.custom_quotes is 'Per-user custom pricing quotes created by admins; billed via Polar ad-hoc checkout.';
...
comment on table public.admin_users is 'Users allowed to access /admin dashboard.';
...
comment on table public.admin_user_snapshots is 'Pre-aggregated user metrics for admin dashboard; refreshed by cron only.';
...
comment on table public.admin_event_log is 'Audit log for admin actions and quote lifecycle events.';
