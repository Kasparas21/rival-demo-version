...
import { appOriginForRequest, authLinkOriginForRequest } from "@/lib/auth/auth-link-origin";
...
describe("appOriginForRequest", () => {
  it("uses localhost origin when the request is local even if NEXT_PUBLIC_APP_URL is production", () => {
    ...
    expect(appOriginForRequest(req)).toBe("http://localhost:3000");
  ...
  it("uses configured app URL for production requests", () => {
    ...
    expect(appOriginForRequest(req)).toBe("https://spy-rival.com");
  });
});
