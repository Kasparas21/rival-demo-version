...
function getMockResponse(
  ...
  const grounding =
    myBrandContext && myBrandContext.trim().length > 0
      ? ` Your positioning notes describe: ${myBrandContext.trim().slice(0, 420)}${myBrandContext.trim().length > 420 ? "…" : ""}`
      : "";
  ...
  return [
    {
      ...
      body: `Based on ${competitorName}'s ad strategy, they focus heavily on top-of-funnel acquisition. Consider analyzing their retargeting creatives to identify gaps in your own funnel.${grounding}`,
      type: "insight",
    },
    {
      title: "Next Step",
...
export function AIInsightChat({
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    const cards = getMockResponse(text, competitorName, myBrandName, myBrandContext);
