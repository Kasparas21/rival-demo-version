...
export function FacebookLogo(
  props: SVGProps<SVGSVGElement> & {
    /** Ensures unique defs when multiple Facebook links render */
    idSuffix?: string;
  }
) {
  const { idSuffix = "d", ...rest } = props;
  const cid = `rival_fb_clip_${idSuffix}`;
  return (
    <svg {...rest} viewBox="0 0 666.667 666.667" aria-hidden>
      <defs>
        <clipPath id={cid} clipPathUnits="userSpaceOnUse">
          <path d="M0 700h700V0H0Z" />
        </clipPath>
      </defs>
      <g clipPath={`url(#${cid})`} transform="matrix(1.33333 0 0 -1.33333 -133.333 800)">
        <path
