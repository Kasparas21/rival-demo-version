...
import { BrandProvider, type Brand } from "@/app/dashboard/brand-context";
...
import {
  ...
  competitorHostFromShellPathname,
...
export function DashboardLayoutInner({
  ...
  const { basePath, isPreviewMode } = useDashboardShell();
  const spyPath = shellPath(basePath, "/spy");
  const competitorPrefix = shellPath(basePath, "/competitor");
  const supabase = useMemo(() => createSupabaseBrowserClient(), []);
  const { state: workspaceState } = useWorkspaceContext({ previewMode: isPreviewMode });
  const isViewer = isPreviewMode || (workspaceState?.isViewer ?? false);
  ...
  const goToBrandHub = useCallback(
    (brand: Brand) => {
      ...
      if (!domain) {
        const href = `${buildCompetitorDashboardPath(WORKSPACE_BRAND_PLACEHOLDER_SLUG, basePath)}?tab=ads%20library&sub=paid-media-settings`;
        router.push(href, { scroll: false });
        return;
      }
      const base = buildCompetitorDashboardPath(domain, basePath);
  ...
  const isOnDemoPath = !isPreviewMode && isDashboardDemoPath(pathname);
  ...
  const pathCompetitorHost = isOnDemoPath
    ...
    : competitorHostFromShellPathname(pathname, basePath);
  ...
  const handleRemoveWatchedCompetitor = useCallback(
    async (competitor: SidebarCompetitor, rowSlugNav: string) => {
      ...
      const onCompetitorView =
        pathname.startsWith(`${competitorPrefix}/`) || pathname === competitorPrefix;
      ...
      if (viewingDeleted) {
        router.replace(spyPath, { scroll: false });
      }
    },
    [pathname, activeCompetitorSlug, activeBrand.id, refreshBillingUsage, refreshSavedCompetitors, router, competitorPrefix, spyPath],
  );
  ...
  return (
    ...
    >
          ...
          <div className={collapsed ? "flex flex-col items-center gap-2.5" : "space-y-1.5"}>
            {sidebarCompetitorRows.map((competitor, competitorIdx) => {
              ...
              const href = isOnDemoPath
                ...
                : buildCompetitorSidebarHref(competitor, basePath);
      ...
      <main className="rival-subtle-scroll relative z-10 flex min-w-0 flex-1 flex-col overflow-x-hidden overflow-y-auto scroll-auto bg-slate-50">
        <ScrapePausedBanner />
        {previewBanner ?? (!isPreviewMode ? <WorkspaceViewerBanner /> : null)}
        <div
          className="relative z-10 flex h-full min-h-0 w-full flex-1 flex-col"
          onClick={() => setIsBrandMenuOpen(false)}
        >
          {!isPreviewMode && !pricingGate && showWelcome && isGenericDashboardLanding(pathname) ? (
              ...
              <div className="mx-4 w-full max-w-[480px] rounded-3xl bg-white p-10 text-center shadow-[0_8px_40px_rgba(0,0,0,0.1)]">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    router.push(spyPath, { scroll: false });
                  }}
