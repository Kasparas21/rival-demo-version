...
/** Selectable `resultsLimit` values for Google (product cap still applies). */
export const GOOGLE_ADS_RESULTS_LIMIT_CHOICES: readonly number[] = Array.from(
  { length: GOOGLE_ADS_LIBRARY_MAX_ITEMS / 50 },
  (_, i) => (i + 1) * 50
);
