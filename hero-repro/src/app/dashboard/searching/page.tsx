...
function SearchingContent() {
  ...
  // Only show platforms we're actually scanning (have identifiers - from discovery or manual input)
  const ids = { ...discoveredIds, ...manualIds };
  const platformsToScan = selectedChannels.filter((ch) => {
    if (ch === "meta") {
      const v = ids.meta ?? ids.metaPageUrl;
  ...
  const runScanAndNavigate = useCallback(
    async (mergedIds: PlatformIdentifier) => {
      setPhase("scanning");
      setScanProgress(0);
      const adsPlatforms = channelsQueryToAdsPlatforms(selectedChannels);
      ...
      let googleResultsLimit = GOOGLE_ADS_LIBRARY_DEFAULT_RESULTS_LIMIT;
      let pinterestCountry = normalizePinterestAdsCountry(undefined);
      try {
        const stored = sessionStorage.getItem("rival-tiktok-ads-region");
        if (stored) tiktokRegion = normalizeTikTokAdsRegion(stored);
        ...
        if (glim) googleResultsLimit = normalizeGoogleAdsResultsLimit(glim);
      ...
      const payload = {
        ...
        googleGetAdDetails: process.env.NEXT_PUBLIC_GOOGLE_AD_DETAILS === "true",
    ...
    [discoveredBrand, displayName, router, selectedChannels]
  );
  ...
  return (
    <div className="flex-1 flex flex-col items-center px-6 py-10 sm:px-12 sm:py-16 pb-20 sm:pb-24 w-full min-h-screen">
      <div className="w-full max-w-5xl flex flex-col items-center">
        ...
        {/* Subtext */}
        <h2 className="text-[17px] sm:text-[20px] font-medium text-[#808080] mb-10 sm:mb-16 text-center tracking-wide transition-all">
          {isDiscovering && "Looking up your competitor..."}
          {isManualNeeded && "We need a bit more info"}
        ...
        <div className={`w-full sm:max-w-2xl flex justify-center px-2 ${isManualNeeded ? "mb-8" : "mb-14 sm:mb-20"}`}>
        ...
        {isDiscovering && (
          <div className="flex flex-col items-center gap-6 w-full">
            {discoveryError ? (
              <>
                <div className="flex items-center gap-1.5 text-[15px] text-red-600 font-medium">
        ...
        {isFound && (
          <div className="mt-10 flex flex-col items-center gap-4 w-full px-2">
            <button
              type="button"
              onClick={() => router.push(buildCompetitorHref())}
